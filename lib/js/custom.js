...
function toggleLang() {
    if (!isAmh) {
        $("#lang-selector").attr("href", "./lib/css/amh.css")
        $(iframe.contentDocument.querySelector("#lang-selector")).attr("href", "./lib/css/amh.css")
        $("#lang-btn").text("Eng")
        isAmh = true;
    } else {
        $("#lang-selector").attr("href", "./lib/css/en.css")
...
function validateInput() {
    ...
    if (type == "") type = null;
    let maxLength = $target.attr("yt-max-length")
    let value = window.event.target.value, newValue = value;
    let alreadyValid = false, keyCode = window.event.keyCode;
    let navButton = keyCode == 8 || keyCode == 9 || keyCode == 37 || keyCode == 39;
    switch (type) {
        case "natural": {
            if (keyCode > 48 && keyCode < 58 || navButton) alreadyValid = true; else { alreadyValid = false; newValue = value }
            break;
        }
        case "whole": {
            if (keyCode > 47 && keyCode < 58 || navButton) alreadyValid = true; else { alreadyValid = false; newValue = value }
            break;
        }
        case "integer": {
            if (keyCode > 47 && keyCode < 58 || keyCode == 189 || navButton) alreadyValid = true; else { alreadyValid = false; newValue = value };
            break;
        }
        case "boolean": {
            if (keyCode == 9) {
            ...
            alreadyValid = false; break;
        }
    }
    if (alreadyValid == true && maxLength > 0 && value.length >= maxLength && (type != "boolean")) {
        newValue = "";
        alreadyValid = false;
    }
    if (alreadyValid == false && value != newValue) {
        window.event.target.value = newValue;
    }
    return alreadyValid;
}
function changeFocus() {
    if (window.event == null || window.event.type != "keyup") return false;
    ...
    if (direction == 1) {
        ...
        if (previousFocus != null) {
            previousFocus.focus()
        } else if (target.previousElementSibling != null) target.previousElementSibling.focus();
    }
}
