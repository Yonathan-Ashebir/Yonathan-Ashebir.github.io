...
function validateInput() {
    ...
    switch (type) {
        case "natural": {
            if (window.event.keyCode > 48 && window.event.keyCode < 58) result = true; else { result = false; newValue = value}
            break;
        }
        case "whole": {
            if (window.event.keyCode > 47 && window.event.keyCode < 58) result = true; else { result = false; newValue = value}
            break;
        }
        case "int": {
            if (window.event.keyCode > 47 && window.event.keyCode < 58 || window.event.keyCode == 189) result = true; else { result = false; newValue = value};
            break;
        }
        case "boolean": {
    ...
    if (result==true&&maxLength > 0 && value.length >= maxLength && (type != "boolean")) {
        newValue = "";
        result =false;
    }
    if (result == false && value != newValue) {
