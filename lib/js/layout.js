    ...
    var
        ...
        */
        translateTo = function (el, x, y, duration = 300, easing = "easeOutQuad", useSystem) {
            ...
            x = computeX(x, el);
            y = computeY(y, el);
            let backup = el.style.transform
            let ind = 0, filtered = "";
            // TODO : ineffective  against internal calls
            while (ind < backup.length) {
                let nextTrans = backup.indexOf(ind, "translate")
                if (nextTrans == -1) nextTrans = backup.length;
                filtered.concat(backup.substring(ind, nextTrans));
                ind = backup.indexOf(")", nextTrans)
                if (ind == -1) ind = backup.length; else { ind++ }
            }
            el.style.transitionDuration = 0 + "ms"
            el.style.transform = filtered;
            ...
            if (useSystem || duration == 0) {
                setTimeout(function () {
                    ...
                    el.style.transform = filtered + " translate(" + dx + "px," + dy + "px)"
                }, 10)
                return;
            }

            let anim = anime({ duration: duration, translateX: dx, translateY: dy, targets: el, easing: easing });
            return anim;
        },
